			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['housekeeping_personnel_id'] && $check_field('set','gender')) || (!form_sub['housekeeping_personnel_id'] && $check_field('add','gender'))">
	...
	export default {
		...
		data() {
			return {
				...
				group_table: "housekeeping_personnel",
				is_password: true,
				...
				form_sub: {
					"gender":'', // 性别
				    "user_id": 0,
					"housekeeping_personnel_id": 0 // ID
				},
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["housekeeping_personnel_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?housekeeping_personnel_id=" + form_sub["housekeeping_personnel_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/housekeeping_personnel/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/housekeeping_personnel/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/housekeeping_personnel/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/housekeeping_personnel/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/housekeeping_personnel/view','get');
