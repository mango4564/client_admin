			...
			<el-col v-if="user_group === '管理员' || $check_field('get','building') || $check_field('add','building') || $check_field('set','building')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="楼栋" prop="building">
					<el-input id="building" v-model="form['building']" placeholder="请输入楼栋"
							  v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','building')) || (!form['dormitory_information_id'] && $check_field('add','building'))" :disabled="disabledObj['building_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','floor') || $check_field('add','floor') || $check_field('set','floor')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="楼层" prop="floor">
					<el-input id="floor" v-model="form['floor']" placeholder="请输入楼层"
							  v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','floor')) || (!form['dormitory_information_id'] && $check_field('add','floor'))" :disabled="disabledObj['floor_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','room_number') || $check_field('add','room_number') || $check_field('set','room_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="房号" prop="room_number">
					<el-input id="room_number" v-model="form['room_number']" placeholder="请输入房号"
							  v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','room_number')) || (!form['dormitory_information_id'] && $check_field('add','room_number'))" :disabled="disabledObj['room_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','room_type') || $check_field('add','room_type') || $check_field('set','room_type')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="房间类型" prop="room_type">
					<el-input id="room_type" v-model="form['room_type']" placeholder="请输入房间类型"
							  v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','room_type')) || (!form['dormitory_information_id'] && $check_field('add','room_type'))" :disabled="disabledObj['room_type_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','room_type')">{{form['room_type']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','number_of_beds') || $check_field('add','number_of_beds') || $check_field('set','number_of_beds')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="床位数" prop="number_of_beds">
					<el-input-number id="number_of_beds" v-model.number="form['number_of_beds']"
						v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','number_of_beds')) || (!form['dormitory_information_id'] && $check_field('add','number_of_beds'))"></el-input-number>
					<div v-else-if="$check_field('get','number_of_beds')">{{form['number_of_beds']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','housekeeping_personnel') || $check_field('add','housekeeping_personnel') || $check_field('set','housekeeping_personnel')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="宿管人员" prop="housekeeping_personnel">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','housekeeping_personnel')) || (!form['dormitory_information_id'] && $check_field('add','housekeeping_personnel'))" :disabled="disabledObj['housekeeping_personnel_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','housekeeping_personnel')">{{form['housekeeping_personnel']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','housekeeping_personnel')) || (!form['dormitory_information_id'] && $check_field('add','housekeeping_personnel'))" id="housekeeping_personnel" v-model="form['housekeeping_personnel']" :disabled="disabledObj['housekeeping_personnel_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','remarks') || $check_field('add','remarks') || $check_field('set','remarks')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="备注" prop="remarks">
					<el-input type="textarea" id="remarks" v-model="form['remarks']" placeholder="请输入备注"
						v-if="user_group === '管理员' || (form['dormitory_information_id'] && $check_field('set','remarks')) || (!form['dormitory_information_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/dormitory_information/add?",
				url_set: "~/api/dormitory_information/set?",
				url_get_obj: "~/api/dormitory_information/get_obj?",
				url_upload: "~/api/dormitory_information/upload?",

				query: {
					"dormitory_information_id": 0,
				},
				...
				form: {
					...
					"room_number":'', // 房号
					"room_type":'', // 房间类型
					"number_of_beds":0, // 床位数
					"housekeeping_personnel": 0, // 宿管人员
					"remarks":'', // 备注
					"dormitory_information_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/dormitory_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/dormitory_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/dormitory_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/dormitory_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/dormitory_information/view','get');
